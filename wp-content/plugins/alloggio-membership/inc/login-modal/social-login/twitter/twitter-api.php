...
if ( ! class_exists( 'AlloggioMembershipTwitterApi' ) ) {
	/**
	 * Class AlloggioMembershipTwitterApi
	 */
	class AlloggioMembershipTwitterApi {
		/**
		 * @var $instance AlloggioMembershipTwitterApi class object
		 */
		...
		/**
		 * @return AlloggioMembershipTwitterApi
		 */
		 ...
		 *
		 * @see AlloggioMembershipTwitterApi::generateSignatureBase()
		 */
		...
		public function obtainRequestToken() {
			...
			if ( is_wp_error( $response ) ) {
				...
				$responseObj->message = esc_html__( 'Internal WP error', 'alloggio-membership' );
			} else {
				...
				if ( ! empty( $responseBody ) ) {
					...
					if ( ( is_array( $responseParsed ) && count( $responseParsed ) ) && ! empty( $responseParsed['oauth_token'] ) && ! empty( $responseParsed['oauth_token_secret'] ) ) {
						...
						if ( empty( $responseObj->redirectUrl ) ) {
							...
							$responseObj->message                  = esc_html__( 'Redirect URL could not be generated', 'alloggio-membership' );
						} else {
							$responseObj->oauth_callback_confirmed = true;
						}
					} else {
						$responseObj->oauth_callback_confirmed = false;
						$responseObj->message                  = esc_html__( 'Couldn\'t connect with Twitter API', 'alloggio-membership' );
					}
				}
			}
		...
		public function buildCurrentPageURI() {
			...
			return alloggio_membership_get_membership_redirect_url();
		}
	}
}
