...
if ( ! function_exists( 'alloggio_core_weather_widget_logic' ) ) {
	function alloggio_core_weather_widget_logic( $atts ) {
		$weather_data = array();
		...
		if ( ! $location ) {
			return alloggio_core_weather_widget_error();
		}
		...
		// set transient name
		$weather_transient_name = 'alloggio_core_' . $city_name_slug . "_" . $days_to_show . "_" . $units . '_' . $locale;
		...
		if ( get_transient( $weather_transient_name ) ) {
			...
			$weather_data['now'] = alloggio_core_weather_ping( $ping_params );
			
			if ( $days_to_show == 5 ) {
				$weather_data['forecast'] = alloggio_core_weather_ping( $ping_params, true );
			}
			
			if ( $weather_data['now'] || $weather_data['forecast'] ) {
				// set the transient, cache for three hours
				set_transient( $weather_transient_name, $weather_data, apply_filters( 'alloggio_core_filter_widget_weather_cache', 1800 ) );
			}
		}
...
if ( ! function_exists( 'alloggio_core_weather_ping' ) ) {
	function alloggio_core_weather_ping( $params, $days = false ) {
...
//		$ping     = str_replace( " ", "", $ping );
		...
		if ( is_wp_error( $ping_get ) ) {
			return alloggio_core_weather_widget_error( $ping_get->get_error_message() );
		}
		...
		if ( isset( $data->cod ) AND $data->cod == 404 ) {
			return alloggio_core_weather_widget_error( $data->message );
		} else {
			return $data;
		}
	}
...
if ( ! function_exists( 'alloggio_core_weather_widget_error' ) ) {
	function alloggio_core_weather_widget_error( $msg = false ) {
		
		if ( ! $msg ) {
			$msg = esc_html__( 'No weather information available', 'alloggio-core' );
		}
