...
if ( ! function_exists( 'alloggio_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function alloggio_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'AlloggioCoreHighlightShortcode';

		return $shortcodes;
	}

	add_filter( 'alloggio_core_filter_register_shortcodes', 'alloggio_core_add_highlight_shortcode' );
}

if ( class_exists( 'AlloggioCoreShortcode' ) ) {
	class AlloggioCoreHighlightShortcode extends AlloggioCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( ALLOGGIO_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'alloggio_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'alloggio-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'alloggio-core' ) );
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Disable Text Line Break', 'alloggio-core' ),
				'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'alloggio-core' ),
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Highlight Text Position', 'alloggio-core' ),
				'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'alloggio-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_color',
				'title'      => esc_html__( 'Highlight Text Color', 'alloggio-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_background_color',
				'title'      => esc_html__( 'Highlight Text Background Color', 'alloggio-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'link',
		...
		public function render( $options, $content = null ) {
			...
			return alloggio_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
