...
if ( ! function_exists( 'alloggio_core_add_font_awesome_to_collection' ) ) {
	function alloggio_core_add_font_awesome_to_collection( $icons ) {
		$icons[] = 'AlloggioCoreFontAwesomePack';
		
		return $icons;
	}
	
	add_filter( 'qode_framework_filter_add_icon', 'alloggio_core_add_font_awesome_to_collection' );
}

if ( class_exists( 'QodeFrameworkIconPack' ) ) {
	class AlloggioCoreFontAwesomePack extends QodeFrameworkIconPack {
		...
		public function get_style_url() {
			return ALLOGGIO_CORE_INC_URL_PATH . '/icons/' . $this->get_base() . '/assets/css/all.min.css';
		}
		...
		function get_new_icons_list() {
			$icons     = '';
			$new_icons = @file_get_contents( ALLOGGIO_CORE_INC_URL_PATH . '/icons/' . $this->get_base() . '/icons.json' );
			...
			if ( ! empty( $new_icons ) ) {
				...
				foreach ( $new_icons as $key => $icon_object ) {
					...
					if ( in_array( 'brands', $icon_object['styles'] ) ) {
						$icon_type = 'fab';
					}
					
					$icons .= '"' . $icon_object['label'] . '" => "' . $icon_type . ' fa-' . $key . '",' . PHP_EOL;
				}
			}
		}
	}
