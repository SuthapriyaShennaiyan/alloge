...
if ( ! function_exists( 'alloggio_core_add_simple_line_icons_to_collection' ) ) {
	function alloggio_core_add_simple_line_icons_to_collection( $icons ) {
		$icons[] = 'AlloggioCoreSimpleLineIconsPack';
		return $icons;
	}
	
	add_filter( 'qode_framework_filter_add_icon', 'alloggio_core_add_simple_line_icons_to_collection' );
}

if ( class_exists( 'QodeFrameworkIconPack' ) ) {
	class AlloggioCoreSimpleLineIconsPack extends QodeFrameworkIconPack {
		...
		public function get_style_url() {
			return ALLOGGIO_CORE_INC_URL_PATH . '/icons/' . $this->get_base() . '/assets/css/' . $this->get_base() . '.min.css';
		}
