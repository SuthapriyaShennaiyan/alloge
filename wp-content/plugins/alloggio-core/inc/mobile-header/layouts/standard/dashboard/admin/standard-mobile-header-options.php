...
if ( ! function_exists( 'alloggio_core_add_standard_mobile_header_options' ) ) {
	function alloggio_core_add_standard_mobile_header_options( $page, $general_tab ) {
		
		$section = $general_tab->add_section_element(
			array(
				'name'       => 'qodef_standard_mobile_header_section',
				'title'      => esc_html__( 'Standard Mobile Header', 'alloggio-core' ),
				'dependency' => array(
					'show' => array(
	...
	add_action( 'alloggio_core_action_after_mobile_header_options_map', 'alloggio_core_add_standard_mobile_header_options', 10, 2 );
}