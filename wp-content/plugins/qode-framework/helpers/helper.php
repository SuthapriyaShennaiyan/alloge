	...
	function qode_framework_wp_kses_html( $type, $content ) {
		switch ( $type ) {
			...
			case 'content':
				...
				break;
			case 'textarea':
				return apply_filters( 'qode_framework_filter_wp_kses_textarea', wp_kses_post( nl2br( wptexturize( $content ) ) ), $type );
				break;
			default:
...
if ( ! function_exists( 'qode_framework_map_shortcode_fields' ) ) {
	...
	function qode_framework_map_shortcode_fields( $default_options, $params ) {
		...
		foreach ( $default_options as $name => $default ) {
			if ( array_key_exists( $name, $atts ) && ( '' !== $atts[ $name ] || ( isset( $default['field_type'] ) && in_array( $default['field_type'], $allowed_empty_types, true ) ) ) ) {
				$out[ $name ] = $atts[ $name ];
			} else {
				$out[ $name ] = is_array( $default ) ? $default['value'] : $default;
			}
		}
...
if ( ! function_exists( 'qode_framework_sanitize_phone_number' ) ) {
	/**
	 * Function that sanitize telephone string
	 *
	 * @param string $phone
	 *
	 * @return string
	 */
	function qode_framework_sanitize_phone_number( $phone ) {
