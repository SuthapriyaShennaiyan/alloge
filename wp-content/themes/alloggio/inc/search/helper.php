...
if ( ! function_exists( 'alloggio_get_search_page_excerpt_length' ) ) {
	/**
	 * Function that return number of characters for excerpt on search page
	 *
	 * @return int
	 */
	function alloggio_get_search_page_excerpt_length() {
		$length = apply_filters( 'alloggio_filter_search_page_excerpt_length', 180 );
		
		return intval( $length );
	}
}

if ( ! function_exists( 'alloggio_override_search_block_templates' ) ) {
	/**
	 * Function that override `core/search` block template
	 *
	 * @see register_block_core_search()
	 */
	function alloggio_override_search_block_templates( $atts ) {
		if ( ! empty( $atts ) && isset( $atts['render_callback'] ) && 'render_block_core_search' === $atts['render_callback'] && function_exists( 'styles_for_block_core_search' ) ) {
			$atts['render_callback'] = 'alloggio_render_block_core_search';
		}
		return $atts;
	}
	add_filter( 'block_type_metadata_settings', 'alloggio_override_search_block_templates' );
}

if ( ! function_exists( 'alloggio_render_block_core_search' ) ) {
	/**
	 ...
	 */
	function alloggio_render_block_core_search( $attributes ) {
		static $instance_id = 0;
		$attributes = wp_parse_args(
			$attributes,
			array(
				'label'      => esc_html__( 'Search', 'alloggio' ),
				'buttonText' => esc_html__( 'Search', 'alloggio' ),
			)
		);
		...
		$label_markup = sprintf(
			...
			$input_id,
			empty( $attributes['label'] ) ? esc_html__( 'Search', 'alloggio' ) : esc_html( $attributes['label'] )
		);
		...
		if ( $show_input ) {
			...
			$input_markup  = sprintf(
				'<input type="search" id="%s" class="search-field %s" name="s" value="%s" placeholder="%s" %s required />',
				$input_id,
		...
		if ( $show_button ) {
			...
			if ( ! $use_icon_button ) {
				...
				$button_internal_markup = '<span aria-hidden="true" class="qodef-search-icon qodef-icon-linea-icons icon-basic-magnifier "></span>';
			}
